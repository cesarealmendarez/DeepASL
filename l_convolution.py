...
class Convolution:
    ...
    def iterate_image_3x3(self, image):
        ...
        for i in range(h - 2):
            for j in range(w - 2):
                image_region_3x3 = image[i:(i + 3), j:(j + 3)]
    ...
    def forward_propagation(self, input):
        ...
        for image_region_3x3, i, j, in self.iterate_image_3x3(input):
            output[i, j] = np.sum(image_region_3x3 * self.filters, axis=(1, 2))
    ...
    def back_propagation(self, d_L_d_out, learn_rate):
        ...
        for image_region_3x3, i, j in self.iterate_image_3x3(self.last_input):
          for f in range(self.num_filters):
            d_L_d_filters[f] += d_L_d_out[i, j, f] * image_region_3x3
